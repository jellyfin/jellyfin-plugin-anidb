...
setting, it is picked up again shortly after being changed, and the plugin page's **Status**
...
Movies are also identified from the downloaded sources, with no file of your own - both map a
good many of them - so a movie another provider has already given a TMDB, IMDb or TVDB id needs
an override only where those two are wrong about it or silent.
